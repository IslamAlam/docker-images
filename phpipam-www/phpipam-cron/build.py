...
parser.add_argument('--name', help='name of docker container', default="phpipam-cron")
...
parser.add_argument('--latest', help='deploy docker container to remote as latest', action='store_true')
parser.add_argument('--flavor', help='flavor (full, light, base) used for docker container', default='full')
...
# arch = '--platform linux/amd64,linux/386,linux/arm64,linux/ppc64le,linux/s390x,linux/arm/v7,linux/arm/v6'


#failed_buildx = call(buildx + " -t "+ versioned_image + " -t " + latest_image + " " 
#            + version_build_arg + " " + flavor_build_arg+ " " + vcs_ref_build_arg + " " + build_date_build_arg + " " + arch + " --file ./")
...
if args.deploy:
    ...
    if args.latest:
        # do not push SNAPSHOT builds as latest version
        # call("docker push " + remote_latest_image)
        failed_buildx = call("docker buildx build" + " -t "+ remote_versioned_image + " -t " + remote_latest_image + " "
            + version_build_arg + " " + flavor_build_arg+ " " + vcs_ref_build_arg + " " + build_date_build_arg + " \\"
            +  buildx + " " + " ./")

    else:
        failed_buildx = call("docker buildx build" + " -t "+ remote_versioned_image + " "
            + version_build_arg + " " + flavor_build_arg+ " " + vcs_ref_build_arg + " " + build_date_build_arg + " \\"
            +  buildx + " " + " ./")
        # call("docker push " + remote_versioned_image)
